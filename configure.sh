...
if [ "$1" == "--gpw" ]; then
  export KPSCONNECT_KEY_PASSWD="$2"
  export KPSCONNECT_KEYSTORE_PASSWD="$3"
fi
...
if [ -z $KPSCONNECT_KEY_PASSWD ]; then
  ...
  export KPSCONNECT_KEY_PASSWD="$KPSCONNECT_KEYPWD"
  export KPSCONNECT_KEYSTORE_PASSWD="$KPSCONNECT_KEYSTOREPWD"
  echo -en "\n"
fi
